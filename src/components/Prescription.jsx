...
const Prescription = ({ caseData, patient, doctor, existingPrescription, isDoctor }) => {
  ...
  // Realtime updates (patient panel auto-updates when doctor changes prescription)
  useEffect(() => {
    const channel = supabase
      .channel('prescriptions')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'prescriptions', filter: `caseId=eq.${caseData.id}` },
        (payload) => {
          if (payload.new) {
            setMedicines(payload.new.medicines || []);
  ...
  const handleSave = async () => {
    ...
    if (validMedicines.length === 0 && !notes.trim()) {
      toast({
        title: 'Prescription cannot be empty.',
        description: 'Please add at least one medicine or a note.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (error) {
      toast({ title: 'Error saving prescription', description: error.message, variant: 'destructive' });
    } else {
  ...
  const prescriptionDate = useMemo(() => {
    return new Date(existingPrescription?.updated_at || existingPrescription?.created_at || Date.now()).toLocaleDateString();
  }, [existingPrescription]);
